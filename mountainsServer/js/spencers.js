...
class ButtonHalf{
	constructor(x, y, width, height, sideObutton, shape = '', fillColor, outlineColor, shapeColor, textOutlineColor, parent, fontSize = 50){
...
class doubleButton{
	constructor(tileData, x,y,width,height,fontSize){
		...
		this.subButtons = [
			new ButtonHalf(x-width/4,y,width/2, height,'L',arrowb,'#0000ff','#000000','#000000',undefined,this,fontSize),
			new ButtonHalf(x+width/4,y,width/2, height,'R',arrowg,'#00ff00','#000000','#000000',undefined,this,fontSize)
		];
		this.tileData = tileData;
		this.highlightColor = "";
	}
...
class Board {
	...
	updateFromServer(recievedBoardState){
		...
		if(boardState.length != recievedBoardState.length || boardState[0].length != recievedBoardState[0].length){ //new boardState if different size
			...
			selected = boardState[Math.floor(boardState.length/2)][Math.floor(line.length/2)]
			}
		}
...
selected = shapes[1][Math.floor(shapes[1].length/2)]
draw();
